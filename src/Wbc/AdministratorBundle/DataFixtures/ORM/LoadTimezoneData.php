...
use Wbc\AdministratorBundle\Entity\Timezone;
use Doctrine\Common\DataFixtures\OrderedFixtureInterface;

class LoadTimezoneData extends AbstractFixture implements OrderedFixtureInterface
{
    public function load(ObjectManager $manager)
    {
        ...
        foreach($timezones as $utc => $t){
            $offset = substr($utc, 4, 6);

            $timezone = new Timezone();
            $timezone->setName($utc);
            $timezone->setPhpName($t);
            $timezone->setUtcOffset($offset);

            $manager->persist($timezone);
            $manager->flush();

            if($t == 'America/Guatemala'){
                $this->addReference('timezone-default', $timezone);
            }
